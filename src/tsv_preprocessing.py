...
if __name__ == "__main__":
    ...
    multi = args.multi
    output_file_name = '/home/pi/packet_convert/convert_to_tsv/prepros_finish/'+host_ip+","+str(resampling_second)+","+file_name.split("/")[-1]
    try:
        t1 = time.time()
        ...
        if multi == 'multi':   
            ...
            with Manager() as manager:
                L = manager.list()
                process_list = []
                total_split_trial = list_split(total_trial,4)
                for each_trial in total_split_trial:
                    p = Process(target=multi_main, args=(file_name, start_time, each_trial, resampling_second, [],[],[], known_protocol, L))
